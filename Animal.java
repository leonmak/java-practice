...
public class Animal {
	...
	// STATIC variables have the same value for every animal object
	// Objects should not have static, as objects should have values specific to it
	// eg. Dog should not know number of animals
	// PROTECTED means that this value can only be accessed by other code in the
	// same package (or by subclasses in other packages)
	...
	// Right-click > Source > generate constructor after fields > after userInput
	public Animal(){
	...
	public void setUniqueID() { // argument can't be long like previous method
		long minNumber = 1;
...
//			i++;
	...
	protected static void guessMyNumber(){
		int number;
		// Do while - execute the code in the braces at least once
		do {
			...
			while(!userInput.hasNextInt()){
				...
				// printf replaces %s w/ 2nd argument. %.2f - floating pt 2 dp.
			}
			number = userInput.nextInt();
		}while(number != 50); // if condition true, run do loop again
...
//		System.out.println("The animal is named " + theDog.getName());
		System.out.println(Animal.printNumbers(10));
		Animal.countTo(99);
